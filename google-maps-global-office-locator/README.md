...
Add the following container to your page:

```html
<div id="global-office-locator" class="gol" data-lang="en"></div>
```

Then:

1. Set the `data-lang` attribute to `en` or `fr`. If omitted or language is unavailable, `en` is used as a fallback.
2. Include `global-office-locator.css` and `global-office-locator.js`.
3. Configure the `offices` array if needed.
4. Initialize the component with default or custom options.
